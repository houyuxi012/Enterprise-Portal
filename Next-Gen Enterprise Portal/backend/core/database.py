...
async def apply_startup_migrations():
    ...
    async with engine.begin() as conn:
        ...
        # User: auth_source & pending_delete_at
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS pending_delete_at TIMESTAMP WITH TIME ZONE;"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS auth_source VARCHAR(50) NOT NULL DEFAULT 'local';"))
        
        # Backfill auth_source for existing directory users
        await conn.execute(text("""
            UPDATE users 
        ...
        # Cleanup legacy employee source field
        await conn.execute(text("ALTER TABLE employees DROP COLUMN IF EXISTS source;"))
