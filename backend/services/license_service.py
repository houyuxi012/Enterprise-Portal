...
class LicenseService:
    ...
    _public_key_cache_raw: Optional[str] = None
    _public_key_cache_obj: Optional[Ed25519PublicKey] = None
    ...
    @classmethod
    def _get_public_key(cls) -> Ed25519PublicKey:
        raw = settings.LICENSE_ED25519_PUBLIC_KEY or ""
        normalized = raw.replace("\r\n", "\n").replace("\r", "\n").strip()
        if not normalized:
            raise LicenseValidationError(
                cls.CODE_INVALID_PAYLOAD,
                "系统未配置 License 公钥（LICENSE_ED25519_PUBLIC_KEY）",
                status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

        expected_fingerprint = (settings.LICENSE_ED25519_PUBLIC_KEY_FINGERPRINT or "").strip().lower()
        if expected_fingerprint.startswith("sha256:"):
            expected_fingerprint = expected_fingerprint.split(":", 1)[1]
        if not expected_fingerprint:
            raise LicenseValidationError(
                cls.CODE_INVALID_PAYLOAD,
                "系统未配置 License 公钥指纹（LICENSE_ED25519_PUBLIC_KEY_FINGERPRINT）",
                status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

        actual_fingerprint = hashlib.sha256(normalized.encode("utf-8")).hexdigest()
        ...
        if cls._public_key_cache_raw == normalized and cls._public_key_cache_obj is not None:
            return cls._public_key_cache_obj
        ...
        if key is None:
            raise LicenseValidationError(
                cls.CODE_INVALID_PAYLOAD,
                "LICENSE_ED25519_PUBLIC_KEY 格式无效",
                status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

        cls._public_key_cache_raw = normalized
        cls._public_key_cache_obj = key
        return key
    ...
    async def has_feature(cls, db: AsyncSession, feature: str) -> bool:
        ...
        runtime = cls._evaluate_runtime_state(state)
        if not runtime["ok"]:
            if state and runtime["reason_code"] in {cls.CODE_EXPIRED, cls.CODE_TIME_ROLLBACK}:
    ...
    async def require_feature(cls, db: AsyncSession, feature: str) -> None:
        ...
        if state and reason_code in {cls.CODE_EXPIRED, cls.CODE_TIME_ROLLBACK}:
            await cls._mark_runtime_transition(
                db,
                state=state,
    ...
    async def get_license_status(
        ...
        *,
        ...
        if not runtime["ok"] and runtime["reason_code"] in {cls.CODE_EXPIRED, cls.CODE_TIME_ROLLBACK}:
            await cls._mark_runtime_transition(
                db,
                state=state,
                reason_code=runtime["reason_code"],
        ...
        return {
            ...
            "installation_id": state.get("installation_id") if state else settings.INSTALLATION_ID,
